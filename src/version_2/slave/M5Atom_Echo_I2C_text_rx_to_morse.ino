...
* 
* Update 2025-11-15
* When, in AtomEchoSPKR.h, I include: #include <driver/i2s_std.h> instead of the original: #include <driver/i2s.h>,
* the sketch continues to restart continuously, with the following output:
* I had boards M5STACK v 3.3.x installed. I removed this version. I re-installed v2.14 (that worked before)
*/
...
/*
  See: https://github.com/m5stack/M5Atom
...
#ifdef USE_ATOM
/*
  IMPORTANT: the M5Atom repo only works with Arduino v.2.3.5, BOARDS MANAGER - M5STACK v2.1.4. 
             See: C:\Users\PaulS2\AppData\Local\Arduino15\packages\m5stack\hardware\esp32\2.1.4
             Do not use v3.x.x or higher!
*/
#include <M5Atom.h>
#else
...
void set_speed() {
  ...
  DEBUG_PRINT(txt0);
  DEBUG_PRINTF("new_speed_idx = %d, le_tone_time_lst-1 = %d, tone_time_lst[%d] = %d\n",
  ...
  if (dest_address != I2C_DEV_ADDR)
    ...
    if (rcvd_set_speed_flag) {
      ...
      */
      ...
      // Temporary print of rx_buffer in hex format
      Serial.print(txt0);
      Serial.print(F("rx_buffer in hex: "));
...
void send_morse() {
  ...
  bool word_end = false;
  bool all_characters_sent = false;
  bool stop = false;
  int unit_count = 0;
  int word_count = 0;
  ...
  */
  ...
  DEBUG_PRINTLN(F("In the lists like: [2,1], 2 represents a dash, 1 respresents a dot"));
  show_delays();
  ...
  while (true) {
    ...
    for (i = text_begin_idx; i < cw_buffer_size; i++) {
      ...
      if (isWordEnd(i, cw_buffer_size, cw_buffer)) { // (n == 32) {
        ...
        DEBUG_PRINT(txt0);
        DEBUG_PRINT(F("searching in morse_tx_dict for character: \'"));
        DEBUG_PRINT(c2);
        DEBUG_PRINTF("\', with value %d\n", c2);
    ...
    if (all_characters_sent) {
      ...
      delay(dly7);
      //unit_count += 7;
      all_characters_sent = false;
    }
    Serial.println();  // New line after the last character (morse code representation)
    DEBUG_PRINTLN(F("All characters sent"));
    DEBUG_PRINT(F("Units sent = "));
    DEBUG_PRINTF("%d\n", unit_count);
